...
def generate_guardduty_information_section(finding):
    ...
    def add_section(label, *keys):
        ...
        if 'severity' in keys:
            sections.append(f"<div class='severity'>{label}: <span>{safe_value}</span></div>")
        else:
            sections.append(f"<div>{label}: <span class='value'>{safe_value}</span></div>")
    ...
    resource_specific_sections = {
        'AccessKey': [('Access Key ID', 'resource', 'accessKeyDetails', 'accessKeyId'), 
                      ('User Name', 'resource', 'accessKeyDetails', 'userName')],
        'Instance': [('Instance ID', 'resource', 'instanceDetails', 'instanceId'), 
                     ('Instance Type', 'resource', 'instanceDetails', 'instanceType')],
        'ECSCluster': [('ECS Cluster Name','resource','containerDetails', 'name')],
        'Lambda': [('Lambda Name','resource','lambdaDetails', 'functionName'),
                   ('Lambda Description','resource','lambdaDetails', 'description')],
        'Container': [('Container Runtime','resource','containerDetails', 'containerRuntime'), ('Container Name','resource','containerDetails', 'name'), ('Container Image','resource','containerDetails', 'image')],
        'EKSCluster': [('EKS Cluster Name','resource','eksClusterDetails', 'name'), ('Workload Details','resource','kubernetesDetails', 'kubernetesWorkloadDetails', 'name'), ('Container Name','resource','containerDetails', 'name')]
    }
    ...
    if action_type:
        sections.append(
            f"Action Type: <span class='value'>{action_type}</span>")

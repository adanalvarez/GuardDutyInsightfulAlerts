...
def get_cloudtrail_information_section(
    ...
    if not event_data_store_id:
        logging.error(
            "Event Data Store does not exist or could not be verified.")
        return ""
...
def get_queries(access_key_id, ip_address, event_data_store_id):
    ...
    queries = {}
    if access_key_id:
        queries["IPs used from the same user"] = f"""
    ...
    if ip_address:
        queries["User identities that have conected from the same IP"] = f"""
...
def execute_queries(queries, event_data_store_arn, event_data_store_region):
    ...
    query_ids = {
        query_data_storage(
            queries[query],
            event_data_store_region): query for query in queries}
    results = {}
...
def create_table_for_query(query, results):
    ...
    table_rows = "".join(
        [f"<tr><td>{html.escape(str(list(result[0].values())[0]))}</td></tr>" for result in results])
    return f"<table><tr><th>{query}</th></tr>{table_rows}</table>"


def check_data_storage_existence(
        event_data_store_arn,
        event_data_store_region):
    """Checks if the specified AWS CloudTrail Event Data Store exists."""
    cloudtrail_client = boto3.client(
        'cloudtrail', region_name=event_data_store_region)
    ...
    try:
        ...
        logging.error(
            f"Error getting Data Storage: {e.response['Error']['Message']}")
        return None
...
def query_data_storage(query, event_data_store_region):
    ...
    try:
        ...
        logging.error(
            f"Error executing query: {e.response['Error']['Message']}")
        return None
...
def is_query_still_running(
    ...
    try:
        ...
        logging.error(
            f"Error getting query state: {e.response['Error']['Message']}")
        return None
